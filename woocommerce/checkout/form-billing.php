	...
	<div class="border border-2 border-light p-4">
		...
		<div class="woocommerce-billing-fields">
			...
			<? if ( wc_ship_to_billing_address_only() && WC()->cart->needs_shipping() ) { ?>
				...
				<h3><i class="bi bi-file-earmark-medical"></i> <? esc_html_e( 'Billing &amp; Shipping', 'woocommerce' ); ?></h3>

			<? } else { ?>

				<h3><i class="bi bi-file-earmark-medical"></i> <? esc_html_e( 'Billing details', 'woocommerce' ); ?></h3>
				...
				<? if ( ! $checkout->is_registration_required() ) { ?>
					...
					<p class="form-row form-row-wide create-account">
						<label class="woocommerce-form__label woocommerce-form__label-for-checkbox checkbox">
							<input class="woocommerce-form__input woocommerce-form__input-checkbox input-checkbox" id="createaccount" <? checked( ( true === $checkout->get_value( 'createaccount' ) || ( true === apply_filters( 'woocommerce_create_account_default_checked', false ) ) ), true ); ?> type="checkbox" name="createaccount" value="1" /> <span><? esc_html_e( 'Create an account?', 'woocommerce' ); ?></span>
						</label>
					</p>
