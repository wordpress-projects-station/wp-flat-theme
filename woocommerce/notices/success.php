...
<?
	...
	if ( ! $notices ) { return; }
		...
		if( is_page('checkout') || is_checkout() ) {
			...
			?><div class=" p-2 border border-2 border-success"><?
			foreach ( $notices as $notice ) 
			?><p class="m-2 woocommerce-message" role="alert" <?= wc_get_notice_data_attr( $notice ); ?>><i class="bi bi-heart"></i> <?= $notice['notice'] ?></p><?
			?></div><?
					...
					<div class="modal-dialog modal-dialog-centered" role="document">
						<div class="modal-content">
							...
							<div class="modal-header">
								<h5 class="modal-title">Operation success:</h5>
