...
<form class="woocommerce-shipping-calculator" action="<?= esc_url( wc_get_cart_url() ); ?>" method="post">
	...
	<? printf( '<small style="margin-top:-10px" ><a href="#" class="shipping-calculator-button">%s</a></small>', esc_html( ! empty( $button_text ) ? $button_text : __( 'Calculate shipping', 'woocommerce' ) ) ); ?>

	<section class="shipping-calculator-form" style="display:none;">

		<? if ( apply_filters( 'woocommerce_shipping_calculator_enable_country', true ) ) { ?>

			<div id="calc_shipping_country_field" class="mt-1 mb-2">
		...
		<? if ( apply_filters( 'woocommerce_shipping_calculator_enable_city', true ) ) { ?>
			<p class="form-row form-row-wide" id="calc_shipping_city_field">
				<input type="text" class="form-control input-text" value="<?= esc_attr( WC()->customer->get_shipping_city() ); ?>" placeholder="<? esc_attr_e( 'City', 'woocommerce' ); ?>" name="calc_shipping_city" id="calc_shipping_city" />
			</p>
		<? } ?>

		<? if ( apply_filters( 'woocommerce_shipping_calculator_enable_postcode', true ) ) { ?>
			<p class="form-row form-row-wide" id="calc_shipping_postcode_field">
				<input type="text" class="form-control input-text" value="<?= esc_attr( WC()->customer->get_shipping_postcode() ); ?>" placeholder="<? esc_attr_e( 'Postcode / ZIP', 'woocommerce' ); ?>" name="calc_shipping_postcode" id="calc_shipping_postcode" />
			</p>
		<? } ?>

		<p><button type="submit" name="calc_shipping" value="1" class="button btn btn-sm btn-outline-dark"><? esc_html_e( 'Update', 'woocommerce' ); ?></button></p>
